...
Since we only want to get the areas for planning areas Changi and Marine Parade, we can filter out *mpsz* to get them.
...
Now, we can plot them out.

```{r}
par(mfrow=c(1,2))
plot(CA["PLN_AREA_N"], main = "Changi")
plot(MP["PLN_AREA_N"], main= "Marine Parade")
```
...
We notice that both *CA_roads* and *MP_roads* are geometry type geometry and not a linestring. We can convert them with the following code chunk:
...
We can now plot this data. In this instance:\
